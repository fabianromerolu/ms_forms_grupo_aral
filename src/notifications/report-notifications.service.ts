...
async function fetchAsBase64(url: string, timeoutMs = 15000): Promise<{ base64: string; filename: string }> {
...
function buildReportHtml(r: AnyObj) {
  ...
  const links = (arr: string[]) => {
    if (!Array.isArray(arr) || !arr.length) return '<em>—</em>';
...
export class ReportNotificationsService {
  ...
  async notifyReportCreated(report: AnyObj) {
    ...
    // ✅ IMPORTANTE: el SDK suele devolver {data, error}
    ...
    this.logger.log(`Resend queued id=${data?.id ?? '—'}`);
  }
}